...
def run_real(policy_path, motion_file):
    """
    面向过程的真机运行主循环 (无历史帧，单帧 114 维)
    """
    print("=" * 60)
    print("初始化 E1-21DOF BeyondMimic 跟踪任务 (无历史帧版本)...")
    ...
    first_frame_onnx = motioninputpos[0, :]
    ...
    # ── 5. 倒计时准备 ──
    wait_seconds = 1.0
    print(f"\n[SUCCESS] 就位完成！倒计时 {int(wait_seconds)} 秒后自动开始动作跟随...")
    wait_time = 0.0
    ...
    try:
        while True:
            ...
            if count_lowlevel % decimation == 0:
                ...
                cur_joint_pos  = (q_onnx - default_pos_onnx).astype(np.float32)
                cur_joint_vel  = dq_onnx.astype(np.float32)
                cur_action     = action_buffer.astype(np.float32)

                # 拼接当前帧特征 (114维)
                obs = np.concatenate([
                    cur_cmd.reshape(-1),
                    cur_ref_ori.reshape(-1),
                    cur_ang_vel.reshape(-1),
                    cur_joint_pos.reshape(-1),
                    cur_joint_vel.reshape(-1),
                    cur_action.reshape(-1)
                ...
                target_q_real = target_pos_onnx[idx_to_real]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--policy_path", type=str, default="/home/saw/droidup/E1_BeyondMimic/logs/rsl_rl/e1_flat/2026-03-20_10-15-20/exported/model_2000.onnx")
