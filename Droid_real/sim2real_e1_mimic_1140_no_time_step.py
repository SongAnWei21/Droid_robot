...
"""Unified sim2real script for E1 robot (10-frame Term-wise History, 1140 obs, 航向对齐完美防爆版)."""
...
def get_yaw_from_quat_w(q):
    """从 [w, x, y, z] 格式的四元数中提取 Yaw 偏航角"""
...
def run_real(policy_path, motion_file):
    print("=" * 60)
    print("初始化 E1-21DOF BeyondMimic 跟踪任务 (1140维, 航向对齐完美版)...")
    ...
    # ==================== 2. 硬编码网络参数 ====================
    print(f"[INFO]: Loading hardcoded parameters & policy {policy_path}")
    joint_seq = [
    ...
    default_pos_real = default_pos_onnx[idx_to_real]
    ...
    # 核心：目标姿态 = 动捕偏移量 + 真机默认蹲姿
    first_frame_onnx = motioninputpos[timestep, :]
    first_frame_real_rel = first_frame_onnx[idx_to_real]
    target_pos_abs_init = first_frame_real_rel + default_pos_real
    ...
    while tt_init < T_init:
        ...
        for i in range(13):
            robot.legCommand.position[i] = s0 * q0_leg[i] + s1 * target_pos_abs_init[i]
        for i in range(8):
            robot.armCommand.position[i] = s0 * q0_arm[i] + s1 * target_pos_abs_init[13 + i]
    ...
    # ── 5. 倒计时准备 ──
    wait_seconds = 1.0
    print(f"\n[SUCCESS] 就位完成！倒计时 {int(wait_seconds)} 秒后 机器人 接管控制权...")
    wait_time = 0.0
    timer_wait = NanoSleep(5) 
    
    while wait_time < wait_seconds:
        start_time = time.perf_counter()
        robot.get_robot_state()
        
        for i in range(13): robot.legCommand.position[i] = target_pos_abs_init[i]
        for i in range(8):  robot.armCommand.position[i] = target_pos_abs_init[13 + i]
        robot.set_robot_command()
    ...
    # ==================== 终极护盾 A：航向 (Yaw) 自动对齐 ====================
    # 消除真机放置方向与动捕数据基准方向的夹角，防止 真机 开局疯狂转体
    robot.get_robot_state()
    base_euler = np.array(robot.legState.imu_euler)
    base_euler[base_euler > math.pi] -= 2 * math.pi
    yaw_real_0 = base_euler[2]
    
    yaw_motion_0 = get_yaw_from_quat_w(motionquat[0, motion_body_idx, :]) # 提取参考的yaw
    delta_yaw = yaw_real_0 - yaw_motion_0 # 算出两者之间的夹角
    
    # 构造 Yaw 补偿四元数 [w, x, y, z]
    q_yaw_corr = np.array([math.cos(delta_yaw / 2.0), 0.0, 0.0, math.sin(delta_yaw / 2.0)], dtype=np.double)
    print(f"[INFO] 航向完美对齐: 动捕_Yaw={yaw_motion_0:.2f}, 真机_Yaw={yaw_real_0:.2f}, 已补偿偏角!")
    # =========================================================================

    # ==================== 终极护盾 B：获取真实下垂姿态并“欺骗”真机 ====================
    ...
    # 反推初始 action_buffer，清零网络的第一下指令惊跳
    ...
    try:
        while True:
            ...
            if count_lowlevel % decimation == 0:
                ...
                # 【核心计算】：先将动捕方向旋转到与真机一致，再计算相对旋转矩阵！
                q02_raw = motionquat[timestep, motion_body_idx, :] 
                q02 = quat_mul_np(q_yaw_corr, q02_raw)  # 完美抵消放置航向角误差
                ...
                # 直接传 obs 给 ONNX 推理模型
                action_onnx = policy.run(None, {input_name: obs})[0][0]
                ...
                # 【终极护盾 C：软启动离合 (Fade-in)】
                FADE_IN_STEPS = 100.0  # 100步 = 2秒
...
if __name__ == '__main__':
    ...
    parser.add_argument("--policy_path", type=str, default="/home/saw/droidup/E1_BeyondMimic/policy/his_1140/model_50k_action.onnx")
