"""Unified sim2real script for E1 robot (10-frame Term-wise History, 1140 obs, 硬编码无TimeStep, 完美真机防爆版)."""
...
def run_real(policy_path, motion_file):
    """
    面向过程的真机运行主循环 (全自动，1140维，硬编码无TimeStep)
    """
    print("=" * 60)
    print("初始化 E1-21DOF BeyondMimic 跟踪任务 (1140维, 完美防摔无TimeStep)...")
    ...
    # 将 default_pos 转换为真实的硬件顺序！这是防炸机的根基！
    default_pos_real = default_pos_onnx[idx_to_real]

    policy = ort.InferenceSession(policy_path)
    # 获取唯一的输入口名称 (用于去掉 time_step)
    ...
    # 提取第 0 帧并加上 default_pos，获得真实的物理关节起步绝对角度
    ...
    # 反推初始 action_buffer，让真机以为自己上一帧发出的就是维持当前物理状态的指令
