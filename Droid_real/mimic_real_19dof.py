...
onnx_mode_path = f"/home/saw/RL/humanoid_robot/Droid/Droid_robot/Droid_real/policles/model_19dof_1.onnx"
# 用来跟踪的动作 NPZ 文件
motion_file_path = f"/home/saw/RL/humanoid_robot/Droid/Droid_robot/Droid_real/motion/dance80_600_19dof.npz"
...
class Sim2Real(RobotBase):
    def __init__(self):
        ...
        # 按照 RealJointOrder 严格排列的 default_joints (不要改！留给 get_obs 和 action 还原用)
        self.default_joints = np.array([
            -0.300, 0.000, 0.000, 0.600, -0.300, 0.000,  # 左腿 6
            -0.300, 0.000, 0.000, 0.600, -0.300, 0.000,  # 右腿 6
            0.000,                                       # 腰部 1
            0.000, 0.000, 0.000,                         # 左臂 3
            0.000, 0.000, 0.000                          # 右臂 3
        ])
        
        # 按照 RealJointOrder 严格提取的 action_scale
        self.action_scale = np.array([
            0.150, 0.150, 0.180, 0.296, 0.296, 0.175,    # 左腿 6
            0.150, 0.150, 0.180, 0.296, 0.296, 0.175,    # 右腿 6
            0.150,                                       # 腰部 1
            0.180, 0.180, 0.175,                         # 左臂 3
            0.180, 0.180, 0.175                          # 右臂 3
        ])

        # 【核心适配】底层 SDK 已经裁减为 6 个手臂关节，直接顺序映射
        self.arm_physical_indices = [0, 1, 2, 3, 4, 5]
        ...
        # ==================== 【新增：屏蔽下半身动作】 ====================
        print("[INFO]: Zeroing out lower body reference motions (Legs + Waist)...")
        # 1. 提取下半身（前13个）的关节名字
        lower_body_joint_names = RealJointOrder[:13]
        
        # 2. 找到这些名字在 NPZ (IsaacLab) 顺序中的真实列索引
        lower_body_indices = [IsaacLabJointOrder.index(name) for name in lower_body_joint_names]
        
        # 3. 将所有帧的下半身目标速度强行设为 0.0
        self.motioninputvel[:, lower_body_indices] = 0.0
        
        # 4. 将下半身的目标位置设为 default_joints 的安全蹲姿 (避免完全直腿摔倒)
        default_isaac = self.default_joints[Real_to_Isaac_indices]
        for idx in lower_body_indices:
            self.motioninputpos[:, idx] = default_isaac[idx]
        # ===================================================================
        ...
        self.timestep = 0
        self.motion_body_idx = 0  # 确保 0 是 NPZ 里代表 torso/pelvis 的索引
    ...
    def init_robot(self):
        ...
        # 补齐硬件命令数组
        while len(self.legCommand.position) < len(self.legState.position):
            self.legCommand.position.append(0.0)
        while len(self.armCommand.position) < len(self.armState.position):
            self.armCommand.position.append(0.0)
            
        T = 2.0  # 2秒平滑过渡时间 (为了真机安全，绝对不能为0)
        dt = 0.002
        tt = 0.0
        timer = NanoSleep(2)

        # ==================== 【关键修改】 ====================
        # 提取第 0 帧 (timestep=0) 的姿态，并从 Isaac 顺序转换为硬件 Real 顺序
        first_frame_isaac = self.motioninputpos[0, :]
        first_frame_real = first_frame_isaac[Isaac_to_Real_indices]
        # ======================================================
        ...
        while tt < T:
            ...
            self.get_robot_state()
            st = min(tt / T, 1.0)
            # 使用余弦平滑函数，保证起步和停止时速度为 0，防止电机过冲
            s0 = 0.5 * (1.0 + math.cos(math.pi * st))
            s1 = 1 - s0
            
            # 平滑插值：目标直接设定为 first_frame_real，而不是 default_joints！
            for i in range(self.legActions):
                self.legCommand.position[i] = s0 * self.legState.position[i] + s1 * first_frame_real[i]
            
            for seq_idx, hw_idx in enumerate(self.arm_physical_indices):
                self.armCommand.position[hw_idx] = s0 * self.armState.position[hw_idx] + s1 * first_frame_real[self.legActions + seq_idx]
                self.armCommand.position[hw_idx] = s0 * self.armState.position[hw_idx] + s1 * first_frame_real[self.legActions + seq_idx]
        ...
        print("[SUCCESS] 已稳稳就位于第一帧姿态！单击手柄 START 开始跟踪跳舞, LT 按压到底急停")
        
        # 等待按键期间，持续下发第一帧指令，让机器人“紧绷肌肉”不软腿
        while (self.rc.state.START == False) and (self.run_flag == True):
            ...
            # 持续死死锁住第一帧姿态
            for i in range(self.legActions):
                self.legCommand.position[i] = first_frame_real[i]
            for seq_idx, hw_idx in enumerate(self.arm_physical_indices):
                self.armCommand.position[hw_idx] = first_frame_real[self.legActions + seq_idx]
    ...
    def get_obs(self, timestep):
        ...
        # 【核心适配】安全读取手臂物理地址
        for seq_idx, hw_idx in enumerate(self.arm_physical_indices):
            q[self.legActions + seq_idx] = self.armState.position[hw_idx]
            dq[self.legActions + seq_idx] = self.armState.velocity[hw_idx]
        ...
        # 计算真实机器人的当前朝向姿态并转换成旋转矩阵误差 (motion_ref_ori_b)
        ...
        q01 = np.array(eq, dtype=np.double)  
        ...
        obs[offset:offset + self.num_actions] = q_isaac - default_isaac  # 19
        offset += self.num_actions
        
        obs[offset:offset + self.num_actions] = dq_isaac  # 19
        offset += self.num_actions   
        
        obs[offset:offset + self.num_actions] = action_isaac  # 19
    ...
    def run(self):
        ...
        # 这里的 timestep 就是 0，因为在 init_robot 里机器人已经摆好了第 0 帧姿态
        while True:
            start_time = time.perf_counter()
            self.get_robot_state()
            ...
            for seq_idx, hw_idx in enumerate(self.arm_physical_indices):
                self.armCommand.position[hw_idx] = self.target_q[self.legActions + seq_idx]
                
            self.set_robot_command()
            
            # 安全递增 timestep，并取模，保证动画无限循环播放防越界
...
if __name__ == '__main__':
    mybot = Sim2Real()
    mybot.init_robot()  
    # 去掉了 time.sleep(1)，因为等待手柄按下 START 的死循环已经起到了阻断作用
    mybot.run()